...
function ENT:MoveSomeWhere(distance)
    ...
    self:MoveToPos( pos, { tolerance = 30, maxage = maxAge, lookahead = 10, repath = 2, draw = true})
end
...
function ENT:MoveToSpot( type )
    ...
    if ( pos ) then
        ...
        if !nav:IsUnderwater() then
            ...
            self:MoveToPos( pos, { tolerance = 30, lookahead = 10, repath = 2, draw = true} )
        end
    end
end
...
local function locogenerator(self, area, fromArea, ladder, elevator, length )
    return 0 end --[[
...
--[[
local oldfunc = CNavArea.IsBlocked
function CNavArea:IsBlocked( teamID, ignoreNavBlockers)
    oldfunc(self, teamID, true)
end
]]
...
function ENT:MoveToPos( pos, options )
    ...
    path:Compute( self, pos, function(...) locogenerator(self, ...) end )
